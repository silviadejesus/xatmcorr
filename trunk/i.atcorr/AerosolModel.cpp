...
void AerosolModel::parse(const float xmud)
{
	...
	if(mie_in.icp >= 4) {
            cout<< (_("mie_in.icp: %ld > 4, will cause internal buffer overflow"), mie_in.icp) <<endl;
	}
	...
	if(mie_in.irsunph >= 50) {
            cout<< (_("mie_in.irsunph: %ld > 50, will cause internal buffer overflow"), mie_in.irsunph) <<endl;
	}
    ...
    default: cout << (_("Unknown aerosol model!")) <<endl;
    }
